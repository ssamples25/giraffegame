...
function Villain(pos) {
    this.pos = pos;
  this.size = new Vector(2.3, 1.5);
 // this.wobble = Math.random() * Math.PI * 2;
  this.speed = new Vector(2, 0);
}
Villain.prototype.type = "villain";
...
Villain.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  //}else if (type == "villain" && this.status == null) {
    //this.status = "lost";
    //this.finishDelay = 1;
  } else if (type == "coin") {
  ...
  if (type == "villain" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  };
